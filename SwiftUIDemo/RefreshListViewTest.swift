...
struct PullToRefresh: UIViewRepresentable {
    ...
    func updateUIView(_ view: UIView, context: Context) {
        DispatchQueue.main.async {
            guard let scrollView = sequence(first: view, next: { $0.superview })
                    ...
                    .flatMap({ $0 as? UIScrollView }) else {
                return
            }
            scrollView.refreshControl = UIRefreshControl()
            scrollView.refreshControl?.addTarget(
                context.coordinator,
                action: #selector(Coordinator.onRefreshAction),
                for: .valueChanged
            )
        }
    }
    ...
    class Coordinator {
        ...
        @objc func onRefreshAction(_ refreshControl: UIRefreshControl) {
...
struct RefreshListViewTest: View {
...
struct RefreshListViewTest_Previews: PreviewProvider {
